/**
 * Import a beast PNG into public/beasts/{kind}.png (magenta keyed, cropped).
 * Usage: npm run import:beast -- wolf path/to/wolf.png [--flip]
 */
...
function resolveSrc(kindName: string): string | null {
  if (srcArg) {
...
async function importKind(kindName: string): Promise<void> {
  const srcPath = resolveSrc(kindName);
  if (!srcPath) {
    throw new Error(`PNG not found for ${kindName}. Pass path: npm run import:beast -- ${kindName} path/to.png`);
  }
  const outPath = join(root, "public", "beasts", `${kindName}.png`);
  ...
  const mode: "magenta" | "black" = isMagenta(raw.data[0], raw.data[1], raw.data[2], raw.data[3])
    ? "magenta"
    : "black";
  ...
  for (let i = 0; i < outData.data.length; i += 4) {
    if (
      outData.data[i + 3] < 16 ||
      isMagenta(outData.data[i], outData.data[i + 1], outData.data[i + 2], outData.data[i + 3])
    ) {
      outData.data[i] = 255;
      outData.data[i + 1] = 0;
...
async function main(): Promise<void> {
  ...
  if (!kind || !(KINDS as readonly string[]).includes(kind)) {
    console.error(`Usage: npm run import:beast -- <kind> [path] [--flip]`);
    console.error(`Kinds: ${KINDS.join(", ")}`);
    process.exit(1);
  }
  await importKind(kind);
}
